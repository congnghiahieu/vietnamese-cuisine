...
const useFoodListQuery = (email: string) =>
    useQuery<Food[]>({
        ...
        queryFn: async () => {
            ...
            let foodList = querySnapshot.docs.map((doc) =>
                doc.data()
            ) as Food[];
            if (!email) {
                return foodList;
...
const Home = () => {
    ...
    const searchedData = (
        ...
        return data.filter(function (foodItem) {
            return foodItem.title.toLowerCase().includes(searchKey.toLowerCase());
        });
    };
        ...
        >
            ...
            {isPending ? (
                ...
                <StyledFlatList
                    ...
                    data={searchedData(data, searchKey)}
...
const FoodCard = ({ title, imageUrlList, loved }: Food) => {
    ...
    const { playSound } = useSound(
        require("../../../assets/sound/love-sound.mp3")
    );
